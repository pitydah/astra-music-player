"""Tests for AlbumInfoBanner — 3 sections: info, tracks, actions."""
...
class TestAlbumInfoBanner:
    ...
    def test_set_album_summary_shows_info(self, qtbot):
        ...
        banner.set_album_summary(summary)
    ...
    def test_set_track_list_shows_tracks(self, qtbot):
        ...
        for i in range(4):
            t = MagicMock()
            t.title = f"Track {i}"
            t.duration = 180.0 + i * 10
            tracks.append(t)
        banner.set_track_list(tracks)
    ...
    def test_signal_play_requested(self, qtbot):
        ...
        qtbot.addWidget(banner)
        results = []
        banner.play_requested.connect(lambda k: results.append(k))
        summary = AlbumSummary(album_key="k1", title="A", artist="B")
        banner.set_album_summary(summary)
        btn = banner._layout.itemAt(2).widget().layout().itemAt(0).widget()
        btn.click()
    ...
    def test_signal_queue_requested(self, qtbot):
        ...
        qtbot.addWidget(banner)
        results = []
        banner.queue_requested.connect(lambda k: results.append(k))
        summary = AlbumSummary(album_key="k1", title="A", artist="B")
        banner.set_album_summary(summary)
        btn = banner._layout.itemAt(2).widget().layout().itemAt(1).widget()
        btn.click()
        assert len(results) > 0
    ...
    def test_signal_details_requested(self, qtbot):
        ...
        qtbot.addWidget(banner)
        results = []
        banner.details_requested.connect(lambda k: results.append(k))
        summary = AlbumSummary(album_key="k1", title="A", artist="B")
        banner.set_album_summary(summary)
        btn = banner._layout.itemAt(2).widget().layout().itemAt(2).widget()
        btn.click()
        assert len(results) > 0
    ...
    def test_track_row_click_emits_filepath(self, qtbot):
        ...
        for _ in range(1):
            ...
            t.title = "Song"
            t.duration = 180.0
            t.filepath = "/path/song.flac"
            tracks.append(t)
        results = []
        banner.track_clicked.connect(lambda f: results.append(f))
        banner.set_track_list(tracks)
        row = banner._track_rows[0]
        ev = QMouseEvent(QEvent.MouseButtonPress, QPointF(0, 0), QPointF(0, 0),
                         Qt.NoButton, Qt.MouseButton.LeftButton, Qt.NoModifier)
        row.mousePressEvent(ev)
        assert len(results) == 1
        assert results[0] == "/path/song.flac"

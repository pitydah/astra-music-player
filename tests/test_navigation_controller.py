...
class TestNavigationController:
    ...
    def test_forward_button_state_updates(self, ctrl, win):
        ctrl.dispatch("albums")
        ctrl.dispatch("artists")
        ctrl.navigate_back()
        win._forward_btn.setEnabled.assert_called_with(True)

    def test_back_button_state_updates(self, ctrl, win):
        ctrl.dispatch("albums")
        ctrl.dispatch("artists")
        win._back_btn.setEnabled.assert_called_with(True)

    def test_build_breadcrumb_with_previous_hub(self, ctrl, win):
        ctrl.dispatch("library")
        ctrl.dispatch("albums")
        breadcrumb = ctrl._build_breadcrumb("Carátulas y navegación visual", "albums")
        assert "Biblioteca" in breadcrumb

    def test_build_breadcrumb_without_previous(self, ctrl, win):
        breadcrumb = ctrl._build_breadcrumb("Subtitle", "home")
        assert breadcrumb == "Subtitle"

    def test_dispatch_dev_sync_calls_devices_page(self, ctrl, win):
        ctrl.dispatch("dev:sync:phone")
        win._show_devices_page.assert_called_with("dev:sync:phone")

    def test_dispatch_unknown_key_does_not_crash(self, ctrl, win):
        ctrl.dispatch("totally_unknown_key")

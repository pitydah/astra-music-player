...
class MichiLinkServer:
    ...
    def mount(cls, handler_class: type, playback=None, player_service=None, window=None):
        ...
        # Patch do_GET and do_POST on the handler
...
class V1_MIXIN:
    ...
    # ── Permission check (reuses existing _require_permission) ──

    @classmethod
    def _require(cls, handler, permission: str) -> bool:
        return handler._require_permission(permission)
    ...
    def handle_get(cls, handler):
        ...
        # Public
        if path == "/api/v1/server/info" or path == "/api/v1/status":
            has_account = bool(srv and srv._local_account and srv._local_account.exists())
            info = ServerInfo(
                name=srv._alias if srv else "Michi Music Player",
            )
            d = info.to_dict()
        ...
        # Library
            ...
            if srv is None or srv._db is None:
                return handler._send_error("No library", 503)
        ...
        # Sync manifest (delegated to existing providers)
        elif path == "/api/v1/sync/manifest":
            if not cls._check_v1_permission(handler, "GET", path):
                return
            if srv is None or srv._manifest_provider is None:
                return handler._send_error("Manifest service not available", 503)
            qs = urllib.parse.parse_qs(
                handler.path.split("?")[1] if "?" in handler.path else "")
            device_id = (qs.get("device_id") or [""])[0]
            if not device_id:
                return handler._send_error("Missing device_id parameter", 400)
            manifest = srv._manifest_provider(device_id)
            if manifest is None:
                return handler._send_error("No manifest for this device", 404)
            ...
            if srv is None or srv._delta_provider is None:
                return handler._send_error("Delta manifest not available", 503)
            qs = urllib.parse.parse_qs(
                handler.path.split("?")[1] if "?" in handler.path else "")
            device_id = (qs.get("device_id") or [""])[0]
            ...
            if delta is None:
                return handler._send_error("No delta for this device", 404)
        ...
        # Events (SSE stub — real implementation in future)
        elif path == "/api/v1/events":
            handler._send_json({"status": "not_implemented", "message": "SSE not yet implemented"})

        else:
            handler._send_error("Not found", 404)
    ...
    def handle_post(cls, handler):
        ...
        # Playback control
            ...
            handler._send_error("Token refresh not yet implemented", 501)
    ...
    def _handle_pair_confirm(cls, handler, srv, body):
        ...
        if not handler._check_rate_limit(client_ip):
            return handler._send_error("Too many attempts, try later", 429)
        try:
            req = V1PairConfirmRequest.from_json(body)
        except Exception:
            return handler._send_error("Invalid request", 400)
        if srv is None:
            return handler._send_error("Server not ready", 503)
    ...
    def _handle_sync_state(cls, handler, srv, body):
        ...
        try:
            ...
            return handler._send_error("Invalid JSON", 400)
    ...
    def _handle_control(cls, handler, body):
        ...
        if not ps and not pb:
            return handler._send_error("Playback service not available", 503)
        try:
            data = json.loads(body)
        except Exception:
            return handler._send_error("Invalid JSON", 400)
        ...
        if action == "play":
            ...
            seek_ms = float(data.get("seek_ms", data.get("value", 0)))
            ...
            return handler._send_error(f"Unknown action: {action}", 400)
        handler._send_json({"status": "ok", "action": action})
    ...
    def _handle_queue_items(cls, handler, body, srv):
        ...
        uris = data.get("uris", [])
        if not uris:
            return handler._send_error("No uris provided", 400)
        # Resolve track_ids to filepaths
        filepaths = []
        for uri in uris:

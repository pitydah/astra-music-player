"""PodcastManager — high-level operations for podcasts, feed management and episode tracking."""
...
class PodcastManager:
    ...
    def add_feed(self, feed_url: str, import_limit: int = 50) -> dict[str, Any]:
        """Add a podcast feed. Parses it and stores shows + episodes."""
        ...
        if show_id:
            self._repo._conn.execute(
                "UPDATE podcast_shows SET episode_count=? WHERE id=?",
                (imported, show_id),
            )
    ...
    def refresh_feed(self, show_id: int, import_limit: int = 50) -> dict[str, Any]:
        """Re-parse a feed and add new episodes."""
        ...
        new_count = 0
        for ep in parsed.episodes[:import_limit]:
            existing_eps = self._repo._conn.execute(
        ...
        self._repo._conn.execute(
            "UPDATE podcast_shows SET episode_count=?, last_updated=?, updated_at=? WHERE id=?",
            (total, now, now, show_id),
        )
        self._repo._conn.commit()
